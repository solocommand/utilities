...
const build = async () => {
  ...
  const { status } = await spawnSync('bash', ['deploy/image.sh', service, version], { stdio: 'inherit' });
...
const deploy = async () => {
  ...
  const { status } = await spawnSync('bash', ['deploy/k8s.sh', service, version, nrid], { stdio: 'inherit' });
